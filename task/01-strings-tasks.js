...
function encodeToRot13(str) {
    ...
    return arr.map(function(char) {
        ...
        if (char === char.toUpperCase()) {
            let index = bigAlphabet.indexOf(char);
            return index + 13 >= 26 ? bigAlphabet[(index + 13) % 26] : bigAlphabet[index + 13];
        } else {
            let index = smallAlphabet.indexOf(char);
            return index + 13 >= 26 ? smallAlphabet[(index + 13) % 26] : smallAlphabet[index + 13];
        }   
    }).join('');
}
...
function isString(value) {
    return {}.toString.call(value).slice(8, -1) === 'String';
}
